...
// [HSS] Codec Tune APP
#define CODEC_TUNE_APP 1
// #define CODEC_TUNE_APP_DEBUG 1

#ifdef CODEC_TUNE_APP
#define  ANALOG_HEADSET_RX_LEFT_GAIN   0x3B
#define  ANALOG_HEADSET_RX_RIGHT_GAIN   0x3C
#define  DIGITAL_RX_LEFT_GAIN   0x84
#define  DIGITAL_RX_RIGHT_GAIN   0x85
#define  DIGITAL_TX_LEFT_GAIN   0x86
#define  DIGITAL_TX_RIGHT_GAIN   0x87 // not used in Galaxy S+
#define  ANALOG_TX_LEFT_GAIN   0x0D
#define  ANALOG_TX_RIGHT_GAIN   0x0E // not used in Galaxy S+
#define  AUXPGA_LEFT_GAIN   0x2B
#define  AUXPGA_RIGHT_GAIN   0x2C

struct class *marimba_class;
EXPORT_SYMBOL(marimba_class);
static struct device *gain_fs;
EXPORT_SYMBOL(gain_fs);
#endif
...
static struct snd_set_codecgain codec_gain;
static struct snd_set_codecgain temp;
static int codec_tune_enable = 0;

struct snd_set_codecgain {
	int analog_headset_rx_left_gain;
	int analog_headset_rx_right_gain;
	int digital_rx_left_gain;
   	int digital_rx_right_gain;
     	int digital_tx_left_gain;
     	int digital_tx_right_gain;
	int analog_tx_left_gain;
   	int analog_tx_right_gain;
	int auxpga_left_gain;
	int auxpga_right_gain;
};

static ssize_t codec_tune_enable_show(struct device *dev,struct device_attribute *attr,char *buf)
{
    return sprintf(buf, "%d\n", codec_tune_enable);
}
static ssize_t codec_tune_enable_store(struct device *dev,struct device_attribute *attr,
		const char *buf,size_t count)
{
    int reg = simple_strtoul(buf, NULL, 10);
	if( ( reg >= 0 ) && ( reg <= 1 ) ) // 0 ~ 1
	{
		codec_tune_enable = reg;
	}
	return count;
}

static ssize_t analog_headset_rx_left_gain_show(struct device *dev,struct device_attribute *attr,char *buf)
{
    return sprintf(buf, "%d\n", temp.analog_headset_rx_left_gain);
}
static ssize_t analog_headset_rx_left_gain_store(struct device *dev,struct device_attribute *attr,
		const char *buf,size_t count)
{
    int reg = simple_strtoul(buf, NULL, 10);
	if( ( reg >= 0 ) && ( reg <= 0xac ) ) // 0 ~ 0xac
	{
		temp.analog_headset_rx_left_gain = reg;
	}
	return count;
}
static ssize_t analog_headset_rx_right_gain_show(struct device *dev,struct device_attribute *attr,char *buf)
{
    return sprintf(buf, "%d\n", temp.analog_headset_rx_right_gain);
}
static ssize_t analog_headset_rx_right_gain_store(struct device *dev,struct device_attribute *attr,
		const char *buf,size_t count)
{
    int reg = simple_strtoul(buf, NULL, 10);
	if( ( reg >= 0 ) && ( reg <= 0xac ) )
	{
		temp.analog_headset_rx_right_gain = reg;
	}
	return count;
}

static ssize_t digital_rx_left_gain_show(struct device *dev,struct device_attribute *attr,char *buf)
{
    return sprintf(buf, "%d\n", temp.digital_rx_left_gain);
}
static ssize_t digital_rx_left_gain_store(struct device *dev,struct device_attribute *attr,
		const char *buf,size_t count)
{
    int reg = simple_strtoul(buf, NULL, 10);
	if( ( reg <= 0x14 ) || ( ( reg >= 0xac ) && ( reg <= 0xff ) ) )    // -84 ~ 20 => [HEX] 0~14, AC~FF
	{
		temp.digital_rx_left_gain = reg;
	}
	return count;
}
static ssize_t digital_rx_right_gain_show(struct device *dev,struct device_attribute *attr,char *buf)
{
    return sprintf(buf, "%d\n", temp.digital_rx_right_gain);
}
static ssize_t digital_rx_right_gain_store(struct device *dev,struct device_attribute *attr,
		const char *buf,size_t count)
{
    int reg = simple_strtoul(buf, NULL, 10);
	if( ( reg <= 0x14 ) || ( ( reg >= 0xac ) && ( reg <= 0xff ) ) )    // -84 ~ 20 => [HEX] 0~14, AC~FF
	{
		temp.digital_rx_right_gain = reg;
	}
	return count;
}

static ssize_t digital_tx_left_gain_show(struct device *dev,struct device_attribute *attr,char *buf)
{
    return sprintf(buf, "%d\n", temp.digital_tx_left_gain);
}
static ssize_t digital_tx_left_gain_store(struct device *dev,struct device_attribute *attr,
		const char *buf,size_t count)
{
    int reg = simple_strtoul(buf, NULL, 10);
	if( ( reg <= 0x1e ) || ( ( reg >= 0x9a ) && ( reg <= 0xff ) ) )     // -66 ~ 30 => [HEX] 0~1E, 9A~FF
	{
		temp.digital_tx_left_gain = reg;
	}
	return count;
}
static ssize_t digital_tx_right_gain_show(struct device *dev,struct device_attribute *attr,char *buf)
{
    return sprintf(buf, "%d\n", temp.digital_tx_right_gain);
}
static ssize_t digital_tx_right_gain_store(struct device *dev,struct device_attribute *attr,
		const char *buf,size_t count)
{
    int reg = simple_strtoul(buf, NULL, 10);
	if( ( reg <= 0x1e ) || ( ( reg >= 0x9a ) && ( reg <= 0xff ) ) )     // -66 ~ 30 => [HEX] 0~1E, 9A~FF
	{
		temp.digital_tx_right_gain = reg;
	}
	return count;
}

static ssize_t analog_tx_left_gain_show(struct device *dev,struct device_attribute *attr,char *buf)
{
    return sprintf(buf, "%d\n", temp.analog_tx_left_gain);
}
static ssize_t analog_tx_left_gain_store(struct device *dev,struct device_attribute *attr,
		const char *buf,size_t count)
{
    int reg = simple_strtoul(buf, NULL, 10);
	if( ( reg >= 0 ) && ( reg <= 3 ) ) // 0 ~ 3
	{
		temp.analog_tx_left_gain = reg;
	}
	return count;
}
static ssize_t analog_tx_right_gain_show(struct device *dev,struct device_attribute *attr,char *buf)
{
    return sprintf(buf, "%d\n", temp.analog_tx_right_gain);
}
static ssize_t analog_tx_right_gain_store(struct device *dev,struct device_attribute *attr,
		const char *buf,size_t count)
{
    int reg = simple_strtoul(buf, NULL, 10);
	if( ( reg >= 0 ) && ( reg <= 3 ) ) // 0 ~ 3
	{
		temp.analog_tx_right_gain = reg;
	}
	return count;
}

static ssize_t auxpga_left_gain_show(struct device *dev,struct device_attribute *attr,char *buf)
{
    return sprintf(buf, "%d\n", temp.auxpga_left_gain);
}
static ssize_t auxpga_left_gain_store(struct device *dev,struct device_attribute *attr,
		const char *buf,size_t count)
{
    int reg = simple_strtoul(buf, NULL, 10);
	if( ( reg >= 0 ) && ( reg <= 0x0f ) ) // 0 ~ 0xF
	{
		temp.auxpga_left_gain = reg;
	}
	return count;
}
static ssize_t auxpga_right_gain_show(struct device *dev,struct device_attribute *attr,char *buf)
{
    return sprintf(buf, "%d\n", temp.auxpga_right_gain);
}
static ssize_t auxpga_right_gain_store(struct device *dev,struct device_attribute *attr,
		const char *buf,size_t count)
{
    int reg = simple_strtoul(buf, NULL, 10);
	if( ( reg >= 0 ) && ( reg <= 0x0f ) ) // 0 ~ 0xF
	{
		temp.auxpga_right_gain = reg;
	}
	return count;
}
		...
		const char *buf,size_t count)
{
	...
	if(reg == 1)
	{
		memcpy(&codec_gain, &temp, sizeof(struct snd_set_codecgain));
	}

	return count;
}

static DEVICE_ATTR(codec_tune_enable, S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP, codec_tune_enable_show, codec_tune_enable_store);
static DEVICE_ATTR(analog_headset_rx_left_gain	, S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP, analog_headset_rx_left_gain_show, analog_headset_rx_left_gain_store);
static DEVICE_ATTR(analog_headset_rx_right_gain	, S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP, analog_headset_rx_right_gain_show, analog_headset_rx_right_gain_store);
static DEVICE_ATTR(digital_rx_left_gain	, S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP, digital_rx_left_gain_show, digital_rx_left_gain_store);
static DEVICE_ATTR(digital_rx_right_gain	, S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP, digital_rx_right_gain_show, digital_rx_right_gain_store);
static DEVICE_ATTR(digital_tx_left_gain	, S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP, digital_tx_left_gain_show, digital_tx_left_gain_store);
static DEVICE_ATTR(digital_tx_right_gain	, S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP, digital_tx_right_gain_show, digital_tx_right_gain_store);
static DEVICE_ATTR(analog_tx_left_gain	, S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP, analog_tx_left_gain_show, analog_tx_left_gain_store);
static DEVICE_ATTR(analog_tx_right_gain	, S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP, analog_tx_right_gain_show, analog_tx_right_gain_store);
static DEVICE_ATTR(auxpga_left_gain	, S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP, auxpga_left_gain_show, auxpga_left_gain_store);
static DEVICE_ATTR(auxpga_right_gain	, S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP, auxpga_right_gain_show, auxpga_right_gain_store);
...
static struct attribute *marimba_attributes[] = {
    &dev_attr_codec_tune_enable.attr,
    &dev_attr_analog_headset_rx_left_gain.attr,
    &dev_attr_analog_headset_rx_right_gain.attr,
    &dev_attr_digital_rx_left_gain.attr,
    &dev_attr_digital_rx_right_gain.attr,
    &dev_attr_digital_tx_left_gain.attr,
    &dev_attr_digital_tx_right_gain.attr,
    &dev_attr_analog_tx_left_gain.attr,
    &dev_attr_analog_tx_right_gain.attr,
    &dev_attr_auxpga_left_gain.attr,
    &dev_attr_auxpga_right_gain.attr,
    &dev_attr_gain_all.attr,
    &dev_attr_save.attr,
    NULL
};

static struct attribute_group marimba_attributes_group = {
    .attrs = marimba_attributes
};
...
{
	int rc;
#ifdef CODEC_TUNE_APP_DEBUG
	pr_err("[HSS][%s] reg = [%X], mask = [%X], val = [%X]\n", __func__, reg, mask, val);
#endif
...
#ifdef CODEC_TUNE_APP
static void marimba_adie_codec_get_gain_from_app(u8 *reg, u8 *val)
{
   switch (*reg)
   {
      // 1. Headset analog gain (0x3b, 0x3c)
      case ANALOG_HEADSET_RX_LEFT_GAIN:
         if ( (*val != 0xac) && (codec_gain.analog_headset_rx_left_gain != 0xff) )
            *val = codec_gain.analog_headset_rx_left_gain;
         break;
      case ANALOG_HEADSET_RX_RIGHT_GAIN:
         if ( (*val != 0xac) && (codec_gain.analog_headset_rx_right_gain != 0xff) )
            *val = codec_gain.analog_headset_rx_right_gain;
         break;
      // 2. Digita rx gain (0x84, 0x85)
      case DIGITAL_RX_LEFT_GAIN:
         if ( codec_gain.digital_rx_left_gain != 0x77)
            *val = codec_gain.digital_rx_left_gain;
         break;
      case DIGITAL_RX_RIGHT_GAIN:
         if ( codec_gain.digital_rx_right_gain != 0x77)
            *val = codec_gain.digital_rx_right_gain;
         break;
      // 3. Digital tx gain (0x86, 0x87)
      case DIGITAL_TX_LEFT_GAIN:
         if ( codec_gain.digital_tx_left_gain != 0x77)
            *val = codec_gain.digital_tx_left_gain;
         break;
      case DIGITAL_TX_RIGHT_GAIN:
         if ( codec_gain.digital_tx_right_gain != 0x77)
            *val = codec_gain.digital_tx_right_gain;
         break;
      // 4. Analog tx gain (0x0d,0x0d(6:5))
      case ANALOG_TX_LEFT_GAIN:
         if ( codec_gain.analog_tx_left_gain != 0xff)
            *val =  ( *val & ~(0x60) ) | ( (codec_gain.analog_tx_left_gain) <<5 & (0x60) );
         break;
      case ANALOG_TX_RIGHT_GAIN:
         if ( codec_gain.analog_tx_left_gain != 0xff)
            *val =  ( *val & ~(0x60) ) | ( (codec_gain.analog_tx_left_gain) <<5 & (0x60) );
         break;
      // 5. AUXPGA rx gain (FM Radio) (0x2b,0x2c(3:1))
       case AUXPGA_LEFT_GAIN:
         if ( (*val & 0x80) && ( codec_gain.auxpga_left_gain != 0xff) )
            *val =  ( *val & ~(0x0f) ) | ( (codec_gain.auxpga_left_gain)  & (0x0f) );
         break;
       case AUXPGA_RIGHT_GAIN:
         if ( (*val & 0x80) && ( codec_gain.auxpga_right_gain != 0xff) )
            *val =  ( *val & ~(0x0f) ) | ( (codec_gain.auxpga_right_gain)  & (0x0f) );
         break;
      default:
         break;
   }
						...
						u32 state)
{
	...
	while (!loop_exit) {
		...
		case ADIE_CODEC_ACTION_ENTRY:
			...
			reg, mask, val);
#ifdef CODEC_TUNE_APP
               if ( (codec_tune_enable) && ( (state == 0x100) || (state == 0x1000) ) )
                  marimba_adie_codec_get_gain_from_app(&reg, &val);
#endif
#ifdef CODEC_TUNE_APP_DEBUG
	         pr_err("[HSS][%s] reg = [%X], mask = [%X], val = [%X]\n", __func__, reg, mask, val);
#endif
...
#ifdef CODEC_TUNE_APP
      codec_gain.analog_headset_rx_left_gain= 0xff;
      codec_gain.analog_headset_rx_right_gain= 0xff;
      codec_gain.digital_rx_left_gain= 0x77;
      codec_gain.digital_rx_right_gain= 0x77;
      codec_gain.digital_tx_left_gain= 0x77;
      codec_gain.digital_tx_right_gain= 0x77;
      codec_gain.analog_tx_left_gain= 0xff;
      codec_gain.analog_tx_right_gain= 0xff;
      codec_gain.auxpga_left_gain= 0xff;
      codec_gain.auxpga_right_gain= 0xff;

      temp.analog_headset_rx_left_gain= 0xff;
      temp.analog_headset_rx_right_gain= 0xff;
      temp.digital_rx_left_gain= 0x77;
      temp.digital_rx_right_gain= 0x77;
      temp.digital_tx_left_gain= 0x77;
      temp.digital_tx_right_gain= 0x77;
      temp.analog_tx_left_gain= 0xff;
      temp.analog_tx_right_gain= 0xff;
      temp.auxpga_left_gain= 0xff;
      temp.auxpga_right_gain= 0xff;

	marimba_class = class_create(THIS_MODULE, "marimba");
	if(IS_ERR(marimba_class))
	{
		printk(KERN_ERR "Failed to create class(marimba)\n");
	}
	gain_fs = device_create(marimba_class, NULL, 0, NULL, "marimba_gain");
	if (IS_ERR(gain_fs))
		printk(KERN_ERR "Failed to create device(marimba_gain)!= %ld\n", IS_ERR(gain_fs));

	if ( (sysfs_create_group(&gain_fs->kobj, &marimba_attributes_group)) < 0)
