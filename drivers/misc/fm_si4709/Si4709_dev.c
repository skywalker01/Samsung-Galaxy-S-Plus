...
if (gpio_tlmm_config(GPIO_CFG(GPIO_FM_INT, 0, GPIO_CFG_INPUT,
		...
		printk("%s: gpio_tlmm_config (gpio=%d) failed\n",
	...
	if (!RDS_Block_Data_buffer) {
		printk("%s: Failed to allocate RDS_Block_Data\n", __func__);
		ret = -ENOMEM;
		goto EXIT;
	}
	...
	if (!RDS_Block_Error_buffer) {
		printk("%s: Failed to allocate RDS_Block_Error\n", __func__);
		ret = -ENOMEM;
...
int Si4709_dev_powerup(void)
{
	...
	if (!(RADIO_ON==Si4709_dev.state.power_state)) {
		if ((ret = powerup()) < 0)
			...
			printk("%s: valid_client_state is eFALSE\n", __func__);
			ret = -1;
		} else {
			/* initial settings */
...
int Si4709_dev_powerdown(void)
{
	...
	printk("Si4709_dev_powerdown called");
	mutex_lock(&(Si4709_dev.lock));

	if (Si4709_dev.valid == eFALSE) {
		printk("%s: valid == eFALSE\n", __func__);
		ret = -1;
	} else if ((ret = powerdown()) < 0)
		printk("%s: powerdown failed\n", __func__);
...
int Si4709_dev_suspend(void)
{
	...
	if (Si4709_dev.valid_client_state == eFALSE) {
		printk("%s: valid_client_state == eFALSE\n", __func__);
		ret = -1;
	}
	mutex_unlock(&(Si4709_dev.lock));
...
int Si4709_dev_band_set(int band)
{
	...
	printk("Si4709_dev_band_set called");
	...
	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_band_set called when DS is invalid");
		ret = -1;
	} else {
		switch (band) {
		...
		if (ret == 0) {
			if ((ret = i2c_write(SYSCONFIG2)) < 0) {
				printk("Si4709_dev_band_set i2c_write 1 failed");
...
int Si4709_dev_ch_spacing_set(int ch_spacing)
{
	...
	printk("Si4709_dev_ch_spacing_set called");
	mutex_lock(&(Si4709_dev.lock));
	sysconfig2 = Si4709_dev.registers[SYSCONFIG2];
	prev_ch_spacing = Si4709_dev.settings.channel_spacing;

	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_ch_spacing_set called when DS is invalid");
		ret = -1;
	} else {
		switch (ch_spacing) {
		...
		if(ret == 0) {
			if ((ret = i2c_write(SYSCONFIG2)) < 0) {
				printk("Si4709_dev_ch_spacing_set i2c_write 1 failed");
...
int Si4709_dev_chan_select(u32 frequency)
{
	...
	printk("Si4709_dev_chan_select called");

	mutex_lock(&(Si4709_dev.lock));

	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_chan_select called when DS is invalid");
		ret = -1;
	} else {
		Si4709_dev.state.seek_state = RADIO_SEEK_ON;
		ret = tune_freq(frequency);
		printk("Si4709_dev_chan_select called1");
		Si4709_dev.state.seek_state = RADIO_SEEK_OFF;
	}
...
int Si4709_dev_chan_get(u32 *frequency)
{
	...
	printk("Si4709_dev_chan_get called");

	mutex_lock(&(Si4709_dev.lock));
	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_chan_get called when DS is invalid");
		ret = -1;
	} else if ((ret = i2c_read(READCHAN)) < 0) {
		printk("Si4709_dev_chan_get i2c_read failed");
	} else {
...
int Si4709_dev_seek_up(u32 *frequency)
{
	...
	printk("Si4709_dev_seek_up called");

	mutex_lock(&(Si4709_dev.lock));

	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_seek_up called when DS is invalid");
		ret = -1;
	} else {
		Si4709_dev.state.seek_state = RADIO_SEEK_ON;
...
int Si4709_dev_seek_down(u32 *frequency)
{
	...
	printk("Si4709_dev_seek_down called");

	mutex_lock(&(Si4709_dev.lock));

	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_seek_down called when DS is invalid");
		ret = -1;
	} else {
		Si4709_dev.state.seek_state = RADIO_SEEK_ON;
...
int Si4709_dev_RSSI_seek_th_set(u8 seek_th)
{
	...
	printk("Si4709_dev_RSSI_seek_th_set called");
	mutex_lock(&(Si4709_dev.lock));
	sysconfig2 = Si4709_dev.registers[SYSCONFIG2];

	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_RSSI_seek_th_set called when DS is invalid");
		ret = -1;
	}
	else {
		...
		if ((ret = i2c_write( SYSCONFIG2 )) < 0) {
			printk("Si4709_dev_RSSI_seek_th_set i2c_write 1 failed");
			Si4709_dev.registers[SYSCONFIG2] = sysconfig2;
		}
	}
...
int Si4709_dev_seek_SNR_th_set(u8 seek_SNR)
{
	...
	printk("Si4709_dev_seek_SNR_th_set called");

	mutex_lock(&(Si4709_dev.lock));

	sysconfig3 = Si4709_dev.registers[SYSCONFIG3];
	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_seek_SNR_th_set called when DS is invalid");
		ret = -1;
	}
	else {
		...
		if ((ret = i2c_write( SYSCONFIG3 )) < 0) {
			printk("Si4709_dev_seek_SNR_th_set i2c_write 1 failed");
...
int Si4709_dev_seek_FM_ID_th_set(u8 seek_FM_ID_th)
{
	...
	printk("Si4709_dev_seek_FM_ID_th_set called");
	...
	if (Si4709_dev.valid == eFALSE) {
		...
		SYSCONFIG3_BITSET_RESERVED(&Si4709_dev.registers[SYSCONFIG3]);
		if ((ret = i2c_write( SYSCONFIG3 )) < 0) {
			printk("Si4709_dev_seek_FM_ID_th_set i2c_write 1 failed");
			sysconfig3 = Si4709_dev.registers[SYSCONFIG3];
		}
	}
...
int Si4709_dev_cur_RSSI_get(rssi_snr_t *cur_RSSI)
{
	...
	printk("Si4709_dev_cur_RSSI_get called");
	mutex_lock(&(Si4709_dev.lock));

	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_cur_RSSI_get called when DS is invalid");
		ret = -1;
	} else {
		if ((ret = i2c_read(STATUSRSSI)) < 0) {
			printk("Si4709_dev_cur_RSSI_get i2c_read 1 failed");
		} else {
...
int Si4709_dev_device_id(device_id *dev_id)
{
	...
	printk("Si4709_dev_device_id called");

	mutex_lock(&(Si4709_dev.lock));

	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_device_id called when DS is invalid");
		ret = -1;
	} else {
		if ((ret = i2c_read(DEVICE_ID)) < 0) {
			printk("Si4709_dev_device_id i2c_read failed");
		}
		else {
...
{
	int ret = 0;
	printk("Si4709_dev_chip_id called");

	mutex_lock(&(Si4709_dev.lock));

	if (Si4709_dev.valid == eFALSE)	{
		printk("Si4709_dev_chip_id called when DS is invalid");
		ret = -1;
	}
	else {
		if ((ret = i2c_read(CHIP_ID)) < 0) {
		printk("Si4709_dev_chip_id i2c_read failed");
		} else {
...
int Si4709_dev_sys_config2(sys_config2 *sys_conf2)
{
	...
	printk("Si4709_sys_config2 called");

	mutex_lock(&(Si4709_dev.lock));

	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_sys_config2 called when DS is invalid");
		ret = -1;
	} else {
		if ((ret = i2c_read(SYSCONFIG2)) < 0) {
			printk("Si4709_sys_config2 i2c_read failed");
		} else {
			sys_conf2->rssi_th =
...
int Si4709_dev_sys_config3(sys_config3 *sys_conf3)
{
	...
	printk("Si4709_sys_config3 called");

	mutex_lock(&(Si4709_dev.lock));

	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_sys_config3 called when DS is invalid");
		ret = -1;
	} else {
		if ((ret = i2c_read(SYSCONFIG3)) < 0) {
			printk("Si4709_sys_config3 i2c_read failed");
		}
		else {
...
int Si4709_dev_status_rssi(status_rssi *status)
{
	...
	printk("Si4709_dev_status_rssi called");

	mutex_lock(&(Si4709_dev.lock));

	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_status_rssi called when DS is invalid");
		ret = -1;
	} else {
		if ((ret = i2c_read(STATUSRSSI)) < 0) {
			printk("Si4709_sys_config3 i2c_read failed");
...
int Si4709_dev_sys_config2_set(sys_config2 *sys_conf2)
{
	...
	printk("Si4709_dev_sys_config2_set called");
	...
	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_sys_config2_set called when DS is invalid");
		ret = -1;
	} else {
		...
		if ((ret = i2c_write( SYSCONFIG2 )) < 0) {
			printk("Si4709_dev_sys_config2_set i2c_write 1 failed");
...
int Si4709_dev_sys_config3_set(sys_config3 *sys_conf3)
{
	...
	printk("Si4709_dev_sys_config3_set called");
	...
	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_sys_config3_set called when DS is invalid");
		ret = -1;
	} else {
		...
		if ((ret = i2c_write( SYSCONFIG3)) < 0) {
			printk("Si4709_dev_sys_config3_set i2c_write 1 failed");
...
int Si4709_dev_power_config(power_config *pow_conf)
{
	...
	printk("Si4709_dev_power_config called");
	mutex_lock(&(Si4709_dev.lock));

	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_power_config called when DS is invalid");
		ret = -1;
	} else {
		if ((ret = i2c_read(POWERCFG)) < 0) {
			printk("Si4709_dev_power_config i2c_read failed");
		} else {
...
int Si4709_dev_AFCRL_get(u8 *afc)
{
	...
	printk("Si4709_dev_AFCRL_get called");

	mutex_lock(&(Si4709_dev.lock));

	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_AFCRL_get called when DS is invalid");
		ret = -1;
	} else {
		if ((ret = i2c_read(STATUSRSSI)) < 0) {
			printk("Si4709_dev_AFCRL_get i2c_read failed");
		}
...
int Si4709_dev_DE_set(u8 de_tc)
{
	...
	printk("Si4709_dev_DE_set called");
	...
	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_DE_set called when DS is invalid");
		ret = -1;
	} else {
		switch(de_tc) {
		...
		if (ret == 0)
			if ((ret = i2c_write(SYSCONFIG1)) < 0) {
				printk("Si4709_dev_DE_set i2c_write failed");
...
int Si4709_dev_reset_rds_data()
{
	...
	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_reset_rds_data called when DS is invalid");
		ret = -1;
	} else {
		RDS_Buffer_Index_write = 0;
  		RDS_Buffer_Index_read = 0;
...
int Si4709_dev_VOLEXT_ENB(void)
{
	...
	printk("Si4709_dev_VOLEXT_ENB called");

	mutex_lock(&(Si4709_dev.lock));

	sysconfig3 = Si4709_dev.registers[SYSCONFIG3];
	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_VOLEXT_ENB called when DS is invalid");
		ret = -1;
	} else {
		SYSCONFIG3_BITSET_VOLEXT_ENB(&Si4709_dev.registers[SYSCONFIG3]);
		SYSCONFIG3_BITSET_RESERVED(&Si4709_dev.registers[SYSCONFIG3]);
		if ((ret = i2c_write( SYSCONFIG3 )) < 0) {
			printk("Si4709_dev_VOLEXT_ENB i2c_write failed");
			Si4709_dev.registers[SYSCONFIG3] = sysconfig3;
		}
	}
...
int Si4709_dev_VOLEXT_DISB(void)
{
	...
	printk("Si4709_dev_VOLEXT_DISB called");
	...
	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_VOLEXT_DISB called when DS is invalid");
		ret = -1;
	} else {
		SYSCONFIG3_BITSET_VOLEXT_DISB(&Si4709_dev.registers[SYSCONFIG3]);
		SYSCONFIG3_BITSET_RESERVED(&Si4709_dev.registers[SYSCONFIG3]);
		if ((ret = i2c_write( SYSCONFIG3 )) < 0) {
			printk("Si4709_dev_VOLEXT_DISB i2c_write failed");
			Si4709_dev.registers[SYSCONFIG3] = sysconfig3;
		}
	}
...
int Si4709_dev_volume_set(u8 volume)
{
	...
	printk("Si4709_dev_volume_set called");
	...
	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_volume_set called when DS is invalid");
		ret = -1;
	}
	else {
		...
		if ((ret = i2c_write( SYSCONFIG2 )) < 0) {
			printk("Si4709_dev_volume_set i2c_write failed");
			Si4709_dev.registers[SYSCONFIG2] = sysconfig2;
		}
	}
...
int Si4709_dev_volume_get(u8 *volume)
{
	...
	printk("Si4709_dev_volume_get called");

	mutex_lock(&(Si4709_dev.lock));

	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_volume_get called when DS is invalid");
		ret = -1;
	} else {
...
int Si4709_dev_DSMUTE_ON(void)
{
	...
	printk("Si4709_dev_DSMUTE_ON called");
	...
	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_DSMUTE_ON called when DS is invalid");
		ret = -1;
	} else {
...
int Si4709_dev_DSMUTE_OFF(void)
{
	...
	printk("Si4709_dev_DSMUTE_OFF called");
	...
	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_DSMUTE_OFF called when DS is invalid");
		ret = -1;
	} else {
...
int Si4709_dev_MUTE_ON(void)
{
	...
	printk("Si4709_dev_MUTE_ON called");

	mutex_lock(&(Si4709_dev.lock));
	powercfg = Si4709_dev.registers[POWERCFG];
	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_MUTE_ON called when DS is invalid");
		ret = -1;
	} else {
		...
		if ((ret = i2c_write( POWERCFG )) < 0) {
			printk("Si4709_dev_MUTE_ON i2c_write failed");
			Si4709_dev.registers[POWERCFG] = powercfg;
		}
	}
	mutex_unlock(&(Si4709_dev.lock));
...
int Si4709_dev_MUTE_OFF(void)
{
	...
	printk("Si4709_dev_MUTE_OFF called");

	mutex_lock(&(Si4709_dev.lock));

	powercfg = Si4709_dev.registers[POWERCFG];
	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_MUTE_OFF called when DS is invalid");
		ret = -1;
	} else {
		...
		if ((ret = i2c_write( POWERCFG )) < 0) {
			printk("Si4709_dev_MUTE_OFF i2c_write failed");
			Si4709_dev.registers[POWERCFG] = powercfg;
		}
	}
...
int Si4709_dev_MONO_SET(void)
{
	...
	printk("Si4709_dev_MONO_SET called");
	...
	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_MONO_SET called when DS is invalid");
		ret = -1;
	} else {
		...
		if ((ret = i2c_write( POWERCFG )) < 0) {
			printk("Si4709_dev_MONO_SET i2c_write failed");
			Si4709_dev.registers[POWERCFG] = powercfg;
		}
	}
...
int Si4709_dev_STEREO_SET(void)
{
	...
	printk("Si4709_dev_STEREO_SET called");

	mutex_lock(&(Si4709_dev.lock));
	powercfg = Si4709_dev.registers[POWERCFG];
	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_STEREO_SET called when DS is invalid");
		ret = -1;
	}
	else {
		...
		if ((ret = i2c_write(POWERCFG)) < 0) {
			printk("Si4709_dev_STEREO_SET i2c_write failed");
			Si4709_dev.registers[POWERCFG] = powercfg;
		}
	}
...
int Si4709_dev_RDS_ENABLE(void)
{
	...
	printk("Si4709_dev_RDS_ENABLE called");
	...
	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_RDS_ENABLE called when DS is invalid");
		ret = -1;
	} else {
		SYSCONFIG1_BITSET_RDS_HIGH(&Si4709_dev.registers[SYSCONFIG1]);
#ifdef RDS_INTERRUPT_ON_ALWAYS
		...
		SYSCONFIG1_BITSET_RESERVED( &Si4709_dev.registers[SYSCONFIG1]);
		if ((ret = i2c_write(SYSCONFIG1)) < 0) {
			printk("Si4709_dev_RDS_ENABLE i2c_write failed");
			Si4709_dev.registers[SYSCONFIG1] = sysconfig1;
		}
#ifdef RDS_INTERRUPT_ON_ALWAYS
		else
...
int Si4709_dev_RDS_DISABLE(void)
{
	...
	printk("Si4709_dev_RDS_DISABLE called");
	...
	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_RDS_DISABLE called when DS is invalid");
		ret = -1;
	} else {
		...
		SYSCONFIG1_BITSET_RESERVED( &Si4709_dev.registers[SYSCONFIG1]);
		if ((ret = i2c_write(SYSCONFIG1)) < 0) {
			printk("Si4709_dev_RDS_DISABLE i2c_write failed");
			Si4709_dev.registers[SYSCONFIG1] = sysconfig1;
		}
#ifdef RDS_INTERRUPT_ON_ALWAYS
		else
...
int Si4709_dev_rstate_get(dev_state_t *dev_state)
{
	...
	printk("Si4709_dev_rstate_get called");

	mutex_lock(&(Si4709_dev.lock));

	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_rstate_get called when DS is invalid");
		ret = -1;
	} else {
...
int Si4709_dev_RDS_data_get(radio_data_t *data)
{
	...
	if (Si4709_dev.valid == eFALSE)	{
		...
		if ((ret = i2c_write(SYSCONFIG1)) < 0) {
			printk("Si4709_dev_RDS_data_get i2c_write 1 failed");
			Si4709_dev.registers[SYSCONFIG1] = sysconfig1;
		} else {
			if ((ret=i2c_read(SYSCONFIG1)) < 0)
				printk("Si4709_dev_RDS_data_get i2c_read 1 failed");

			printk("sysconfig1: 0x%x",Si4709_dev.registers[SYSCONFIG1] );
			sysconfig1 = Si4709_dev.registers[SYSCONFIG1];
			...
			if ((ret=i2c_read(STATUSRSSI)) < 0)
				printk("Si4709_dev_RDS_data_get i2c_read 2 failed");

			printk("statusrssi: 0x%x",Si4709_dev.registers[STATUSRSSI] );
			SYSCONFIG1_BITSET_RDSIEN_LOW(&Si4709_dev.registers[SYSCONFIG1]);

			if ((ret = i2c_write(SYSCONFIG1)) < 0) {
				printk("Si4709_dev_RDS_data_get i2c_write 2 failed");
				Si4709_dev.registers[SYSCONFIG1] = sysconfig1;
			} else if(Si4709_dev_wait_flag == WAIT_OVER) {
				Si4709_dev_wait_flag = NO_WAIT;

				if((ret = i2c_read(RDSD)) < 0)
					printk("Si4709_dev_RDS_data_get i2c_read 3 failed");
				else {
					...
					get_cur_chan_freq(&(data->curr_channel), Si4709_dev.registers[READCHAN]);
					printk("curr_channel: %u",data->curr_channel);
					data->curr_rssi = STATUSRSSI_RSSI_SIGNAL_STRENGTH(Si4709_dev.registers[STATUSRSSI]);
					printk("curr_rssi:%u",(u32)data->curr_rssi);
				...
				printk("Si4709_dev_RDS_data_get failure no interrupt or timeout");
				Si4709_dev_wait_flag = NO_WAIT;
				ret = -1;
			}
		}
...
int Si4709_dev_RDS_timeout_set(u32 time_out)
{
	...
	printk("Si4709_dev_RDS_timeout_set called");
	/* convert time_out(in milliseconds) into jiffies */

	jiffy_count = msecs_to_jiffies(time_out);

	printk("jiffy_count%d",jiffy_count);

	mutex_lock(&(Si4709_dev.lock));

	if (Si4709_dev.valid == eFALSE) {
		printk("Si4709_dev_RDS_timeout_set called when DS is invalid");
		ret = -1;
	} else
...
static int powerup(void)
{
	...
	printk("Resetting the Si4709_dev.registers[] array");
	...
	if ((ret = i2c_write(POWERCFG)) < 0) {
		printk("powerup->i2c_write 1 failed");
		Si4709_dev.registers[POWERCFG] = powercfg;
	} else {
...
static int powerdown(void)
{
	...
	if (!(RADIO_POWERDOWN==Si4709_dev.state.power_state)) {
		...
		if ((ret = i2c_write(TEST1)) < 0) {
			printk("powerdown->i2c_write failed");
			Si4709_dev.registers[SYSCONFIG1] = sysconfig1;
			Si4709_dev.registers[POWERCFG] = powercfg;
		...
		printk("Device already Powered-OFF");
...
static int seek(u32 *frequency, int up)
{
	...
	if ((ret = i2c_write(POWERCFG)) < 0) {
		printk("seek i2c_write 1 failed");
		Si4709_dev.registers[POWERCFG] = powercfg;
	} else {
		...
		if (Si4709_dev_wait_flag == SEEK_CANCEL) {
			...
			if ((ret = i2c_write(POWERCFG)) < 0) {
				printk("seek i2c_write 2 failed");
				Si4709_dev.registers[POWERCFG] = powercfg;
			}

			if ((ret = i2c_read(READCHAN)) < 0) {
				printk("seek i2c_read 1 failed");
			}
			else {
		...
		*frequency = 0;
		...
		if ((ret = i2c_read(STATUSRSSI)) < 0)
			...
			if(STATUSRSSI_SEEK_TUNE_STATUS(Si4709_dev.registers[STATUSRSSI]) == COMPLETE) {
				printk("Seek/Tune Status is set to 1 by device");
				if(STATUSRSSI_SF_BL_STATUS(Si4709_dev.registers[STATUSRSSI]) == SEEK_SUCCESSFUL) {
				printk("Seek Fail/Band Limit Status is set to 0 by device ---SeekUp Operation Completed");
				valid_station_found = 1;
				} else
					printk("Seek Fail/Band Limit Status is set to 1 by device ---SeekUp Operation Not Completed");
			} else
				printk("Seek/Tune Status is set to 0 by device ---SeekUp Operation Not Completed");
			...
			if ((ret = i2c_write(POWERCFG)) < 0) {
				...
				if (ret == 0 && valid_station_found == 1) {
					if ((ret = i2c_read(READCHAN)) < 0) {
						printk("seek i2c_read 4 failed");
					} else {
						channel = READCHAN_GET_CHAN(Si4709_dev.registers[READCHAN]);
						*frequency = channel_to_freq(channel);
						printk("Frequency after seek-up is %d \n",*frequency);
					}
				} else {
					printk("Valid station not found \n");
					*frequency = 0;
				}
			}
		}
...
static int tune_freq(u32 frequency)
{
	...
	if ((ret = i2c_write(CHANNEL)) < 0) {
		printk("tune_freq i2c_write 1 failed");
		Si4709_dev.registers[CHANNEL] = channel;
	} else {

		Si4709_dev_wait_flag = TUNE_WAITING;
		printk("Si4709_dev_wait_flag = TUNE_WAITING");
#ifdef TEST_FM
		if ((ret = i2c_read(READCHAN)) < 0) {
			printk("tune_freq i2c_read 1 failed");
		} else {
			read_channel=READCHAN_GET_CHAN(Si4709_dev.registers[READCHAN]);
			printk("curr_channel before tuning = %x",read_channel);
		}
#endif
...
#ifdef TEST_FM
		if ((ret = i2c_read(STATUSRSSI)) < 0 ) {
			printk("tune_freq i2c_read 2 failed");
		} else if (STATUSRSSI_SEEK_TUNE_STATUS(Si4709_dev.registers[STATUSRSSI]) == COMPLETE)
			printk("Seek/Tune Status is set to 1 by device ---Tuning Operation Completed");
		else
			printk("Seek/Tune Status is set to 0 by device ---Tuning Operation Not Completed");
#endif
		...
		CHANNEL_BITSET_RESERVED(&Si4709_dev.registers[CHANNEL]);
		if ((ret = i2c_write(CHANNEL)) < 0) {
			printk("tune_freq i2c_write 2 failed");
			Si4709_dev.registers[CHANNEL] = channel;
		} else {
			do {
				if ((ret = i2c_read(STATUSRSSI)) < 0) {
					printk("tune_freq i2c_read 3 failed");
				break;
				}
			} while (STATUSRSSI_SEEK_TUNE_STATUS(Si4709_dev.registers[STATUSRSSI]) != CLEAR);
		...
		if ((ret = i2c_read(READCHAN)) < 0) {
			...
			read_channel=READCHAN_GET_CHAN(Si4709_dev.registers[READCHAN]);
			printk("curr_channel after tuning= %x",read_channel);
		}
#endif
	}
...
static void get_cur_chan_freq(u32 *frequency, u16 readchan)
{
	...
	u16 channel = 0;
	printk("get_cur_chan_freq called");

	channel = READCHAN_GET_CHAN(readchan);
	printk("read_channel=%x",channel);

	*frequency = channel_to_freq(channel);
	printk("frequency-> %u",*frequency);
}
...
static int  insert_preset(u32 frequency,u8 rssi,u8 *seek_preset_rssi)
{
	...
	printk("si4709 autoseek : insert preset\n");
