...
#define BATT_FULL_CHARGING_VOLTAGE	4200
...
static struct device_attribute godart_battery_attrs[] = {
#ifdef CONFIG_MAX17043_FUEL_GAUGE
...
static int msm_batt_create_attrs(struct device * dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(godart_battery_attrs); i++)
	{
		rc = device_create_file(dev, &godart_battery_attrs[i]);
		if (rc)
			goto failed;
	}
	goto succeed;
...
failed:
	while (i--)
		device_remove_file(dev, &godart_battery_attrs[i]);
...
{
	int i = 0;
	const ptrdiff_t offset = attr - godart_battery_attrs;
...
MODULE_VERSION("1.0");
MODULE_ALIAS("platform:godart_battery");
